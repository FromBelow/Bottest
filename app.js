...
const wallet		=		new QIWI("b9cfb379707bfa52b9941a855c5d5240");

const admins		=		[608824295, 573886476];
...
const bot			=		new Telegram(
	"772875608:AAEC3vJdcJCos6Nlosnx5CHPTVQTsWccYBg",
	{ polling: true }
);
...
setInterval(async () => {
	wallet.getOperationHistory({
		...
		res.data.map(async (operation) => {
			...
			if(!operation.comment.startsWith("wfc")) return;

			let user = await User.findOne({ id: Number(operation.comment.split("wfc")[1]) });
			if(!user) return;
...
Для того, чтобы получить верификацию вы должны пополнить баланс на любую сумму (можно даже 1 рубль)`, {
	...
	if(message.text === "📥 Пополнить") {
		return message.send(`👛 Кошелёк QIWI: <code>+998915054220</code>
📝 Комментарий к платежу: <code>wfc${message.from.id}</code>
...
1️⃣ уровень — <b>10%</b> дохода
2️⃣ уровень — <b>5%</b> дохода

🔗 Ваша ссылка: https://t.me/Tele_Profit_Bot?start=${message.from.id}`, {
			parse_mode: "HTML"
		});
	}
...
📂 Постов на продвижении: <b>${counters.posts}</b>`, {
	...
	if(message.text === "📤 Выплаты" || message.text === "📧 Чат") {
		return message.send(`Чат: @Work_for_CASHbot_CHAT
Канал с выплатами: @Pay_Tele_Profit_Bot`);
	}
	...
	if(admins.indexOf(message.user.id) !== -1) {
		...
		if(query.data.startsWith("withdraw")) {
			...
			await wallet.toWallet({
				...
				comment: "@Tele_Profit_Bot"
			}, (err, success) => {});

			bot.sendMessage(ticket.owner, "Ваша заявка на вывод была одобрена.");
			bot.sendMessage("@Pay_Tele_Profit_Bot", `🤖 <b>Была произведена новая выплата!</b>
...
⭐️ <b>Заходи к нам! Зарабатывай на подписках, просмотрах, приглашениях.</b>`, {
				...
				reply_markup: {
					inline_keyboard: [[
						{ text: "💰 Перейти в бота", url: `https://t.me/Tele_Profit_Bot` }
					]]
				}
			});
